...
#include <SoftwareSerial.h>  // Подключение библиотеки для программного создания последовательного порта
...
void loop() {
  if (Serial.available()) {                       // Если есть данные от монитора порта (ПК)
    ...
    if (input == "0") {                            // Команда "0" — генерация нового ключа
      ...
      Serial.println(F("❓ Неизвестная команда"));  // Обработка некорректного ввода
    }
